...
// static std::vector<int> ParseAttnoFromTargetList(const List* targetList) {
//     ListCell* tl = NULL;
//     std::set<int> attno_set;
//     foreach (tl, targetList) {
//         GenericExprState* gstate = (GenericExprState*)lfirst(tl);
//         Var* variable = (Var*)gstate->xprstate.expr;
//         Assert(variable != NULL); /* if this happens we've messed up */
//         attno_set.insert(variable->varoattno);
//         elog(INFO, "Added attnum: varoattno: %d", variable->varoattno);
//     }
//     std::vector<int> attnos(attno_set.begin(), attno_set.end());
//     return attnos;
// }
...
std::vector<ScanKeyData> parse_conditions(List *exprs, ParamListInfo paramLI) {
    ...
    foreach(lc, exprs)
    {
        ...
        if (list_length(ref_values.column_refs) == 1 && list_length(ref_values.const_values) == 1) {
            ...
            //    case F_SCALARLESEL: {
            //         ScanKeyInit(&skey, col_ref->attr_num, BTLessEqualStrategyNumber, ref_values.opfunc_id, const_value->value);
            //         result.push_back(std::move(skey));
            //         break;
            //    }
               case F_SCALARLTSEL: {
            ...
            //    case F_SCALARGESEL: {
            //         ScanKeyInit(&skey, col_ref->attr_num, BTGreaterEqualStrategyNumber, ref_values.opfunc_id, const_value->value);
            //         result.push_back(std::move(skey));
            //         break;
            //    }
               case F_SCALARGTSEL: {
