...
#ifndef NDEBUG
    elog(WARNING, "Known categories: %s", KnownCategoriesStr().c_str());
#endif
    elog(ERROR, "In %s: unknown category description for %d", function_name, static_cast<int>(category_id));
  }
...
Status::Status(Code code,
  ...
  if (print_stack_trace ||
    ...
    elog(WARNING, "Non-OK status generated: %s", string_rep.c_str());
  }
#endif
}
...
Status Status::CloneAndAddErrorCode(const StatusErrorCode& error_code) const {
  ...
  size_t encoded_size = (size_t)(out - buffer);
  if (encoded_size != new_errors_size) {
    elog(ERROR, "New error codes size is expected to be %lu but %lu bytes were encoded", new_errors_size, encoded_size);
  }
  return Status(State::Create(
